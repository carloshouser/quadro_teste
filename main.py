...
def render_home():
    ...
    st.write(f'{usuario}, consulte as designações de sua congregação de forma rápida e simples.')
    ...
    st.write(f'{usuario}, acesse os territórios de sua congregação de forma bem objetiva.')
...
def render_visualizar():
    ...
    with open(quadro["arquivo"], "rb") as file:
        if quadro["arquivo"].endswith(".png"):
            st.image(file.read(), caption=quadro["titulo"], use_container_width=True)
